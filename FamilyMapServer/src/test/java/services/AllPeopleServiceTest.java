...
class AllPeopleServiceTest {
    ...
    void servePass() throws Exception {
        try {
            ...
            //Make sure that the logins have the same amount of load events that we
            //gave it. Not 4!!!
            LoginService loginService = new LoginService();
            r = loginService.serve(login1);
            GoodLogin gL = ((GoodLogin) r);
            auth = gL.getAuthToken();
            r = allPeopleService.serve(auth);
            AllPeople allPeople2 = ((AllPeople) r);
            System.out.println(allPeople2.size());
            assertEquals(1, allPeople2.size());
    ...
    void serveFail() throws Exception {
        try {
            ...
            //This will fill it with people, an authToken, and a user and events.
            assertEquals(r.getClass(), GoodLogin.class);
            //Let's make sure we filled the persons return an error message upon a bad auth.
            AllPeopleService allPeopleService = new AllPeopleService();
            Response allEvents = allPeopleService.serve("This is a bad authToken");
            assertEquals(allEvents.getClass(), ErrorMessage.class);
